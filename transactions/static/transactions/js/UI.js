...
export function renderHTML(transactionData) {
  ...
  console.log('Inner HTML:', transactionRowContainer.innerHTML);
  ...
  if (transactionData.length == 0) {
    transactionRowContainer.innerHTML = `
      <tr>
        <td colspan="5" class="text-center py-5">
            <span class="w-100">No data, please <a href="{% url 'upload:file_upload' %}">
                upload your transactions here</a>
            </span>
        </td>
      </tr>
    `;
    ...
    transactionData.forEach((data) => {
      ...
      const transactionEditBtn = card.querySelector('.transaction-action .edit-link');
      const transactionDeleteBtn = card.querySelector('.transaction-action .delete-link');
...
export function renderStatsBar(data) {
  const totalElement = document.querySelector('#stats-total-display');
  const dateRangeElement = document.querySelector('#date-range-display');
  const totalTransactionsElement = document.querySelector('#transaction-count-display');
